...
export default function ExploreMenuModal({ open, onClose, onRequireAuth }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[430px] rounded-3xl bg-white/95 dark:bg-[#0c0c12]/95 discuss:bg-[#0c0c12]/95 backdrop-blur-xl border border-neutral-200/80 dark:border-neutral-800/80 dark:border-[#262626] p-0 overflow-hidden shadow-2xl animate-fade-in">
        ...
        <div className="p-6 pb-4 border-b border-neutral-100 dark:border-neutral-800 dark:border-[#262626] relative">
          <span className="text-[10px] font-black tracking-widest text-[#0095F6] text-[#0095F6] font-mono block mb-1 select-none">
            ◈ SYSTEM_GATEWAY
          </span>
          <h2 className="text-xl font-black text-neutral-900 dark:text-neutral-50 dark:text-white leading-none">
            Explore Discuss
          </h2>
          <p className="text-xs text-neutral-500 dark:text-neutral-400 dark:text-neutral-400 mt-2.5 font-medium leading-relaxed">
                ...
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-extrabold text-[13.5px] text-neutral-800 dark:text-neutral-100 dark:text-neutral-100 font-mono group-hover:text-[#0095F6] discuss:group-hover:text-[#EF4444] transition-colors leading-tight">
                        {item.title}
                      </h3>
                      {item.badge && (
                        <span className="text-[8.5px] font-extrabold font-mono px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800/80 discuss:bg-neutral-800/60 text-neutral-400 dark:text-neutral-500 border border-neutral-200/50 dark:border-neutral-700/50 dark:border-[#262626] uppercase select-none">
                          {item.badge}
                        </span>
                      )}
                    </div>
                    <p className="text-[11px] text-neutral-500 dark:text-neutral-400 dark:text-neutral-400 mt-1.5 truncate font-sans font-medium">
                      {item.description}
                    </p>
                  </div>
                </div>
                <ChevronRight className="w-4 h-4 text-neutral-300 dark:text-neutral-600 discuss:text-[#404040] group-hover:text-[#0095F6] discuss:group-hover:text-[#EF4444] group-hover:translate-x-1.5 transition-all duration-300 shrink-0" />
              </button>
            );
          })}
        </div>
