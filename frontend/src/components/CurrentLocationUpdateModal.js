import { CheckCircle2, Loader2, MapPin, RefreshCw, ShieldAlert } from 'lucide-react';
      ...
      <div className={`w-full max-w-sm p-5 shadow-2xl animate-in zoom-in-95 duration-200 ${cardClass}`}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-black uppercase tracking-wide">Update Current Location</h3>
        ...
        <div className="mt-5 flex gap-2">
          ...
          {!isLoading && !isSuccess && !isBlocked && !isError && (
            <button
              type="button"
              onClick={onConfirm}
              className={`flex-1 text-xs font-black uppercase px-3 py-2.5 transition-all active:scale-95 ${primaryButtonClass}`}
            >
              Confirm
            </button>
          )}
