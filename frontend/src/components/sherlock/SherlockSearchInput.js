  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-4">
      <form onSubmit={handleSubmit} className="relative group z-10">
        ...
        <div className="relative flex items-center bg-white/70 dark:bg-[#121212]/80 discuss:bg-[#0c0c12]/90 border border-neutral-200 dark:border-neutral-800 discuss:border-white/10 rounded-2xl p-2.5 backdrop-blur-xl shadow-xl transition-all focus-within:border-blue-500/50 discuss:focus-within:border-red-500/50 focus-within:ring-4 focus-within:ring-blue-500/10 discuss:focus-within:ring-red-500/10">
          <Search className="w-6 h-6 text-blue-600 discuss:text-red-500 ml-3 shrink-0" />
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="maaz123, dev_user (comma for bulk)"
            className="flex-1 bg-transparent border-none outline-none px-4 py-3 text-neutral-900 dark:text-neutral-50 discuss:text-white placeholder-neutral-400 font-mono text-lg"
            disabled={isLoading}
          />
          <button
            type="button"
            onClick={() => setShowAdvanced(!showAdvanced)}
            className="p-3 text-neutral-400 hover:text-blue-600 discuss:hover:text-red-500 transition-colors shrink-0"
            title="Advanced Options"
          >
            <Settings className={`w-5 h-5 transition-transform duration-300 ${showAdvanced ? 'rotate-90 text-blue-600 discuss:text-red-500' : ''}`} />
          </button>
          <button
            type="submit"
            disabled={!query.trim() || isLoading}
            className="ml-2 bg-blue-600 hover:bg-blue-500 discuss:bg-red-500 discuss:hover:bg-red-400 text-white px-5 py-3 rounded-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 font-bold group/btn shadow-[0_4px_14px_rgba(37,99,235,0.39)] discuss:shadow-[0_4px_14px_rgba(239,68,68,0.39)] shrink-0"
          >
            {isLoading ? (
