...
import { Search, Plus, Share2, Pencil, Trash2, ShieldCheck, ExternalLink, Briefcase, Building, MapPin, Calendar, Clock, Star, ArrowUp } from 'lucide-react';
...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToJobs((data) => {
      setJobs(data);
      try {
        localStorage.setItem('discuss_fast_jobs', JSON.stringify(data));
      } catch (e) {}
      setIsSyncing(false);
    });
    return () => unsubscribe();
  ...
  const handleGuestSkip = () => {
    if (pendingApplyJob) {
      window.open(pendingApplyJob.applyLink, '_blank', 'noopener,noreferrer');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] dark:bg-[#0F172A] discuss:bg-[#121212] pb-28">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-neutral-500 dark:text-neutral-400 discuss:text-[#9CA3AF] mt-1">
              Find your next career move curated by Discuss.
            </p>
          </div>
      ...
      {showGuestPopup && pendingApplyJob && (
        <GuestApplyPopup 
          open={showGuestPopup} 
          onClose={() => { setShowGuestPopup(false); setPendingApplyJob(null); }} 
