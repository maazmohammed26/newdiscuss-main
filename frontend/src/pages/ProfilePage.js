...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] dark:bg-[#0F172A] discuss:bg-[#121212]">
            ...
            <div className="mt-4 space-y-4">
                ...
                <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/20 rounded-xl p-4">
                  ...
                  <div className="space-y-2">
                    {receivedRequests.map((request) => {
                      ...
                      /*SecurityHandlers*/ const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; /*SecurityHandlers*/ const initials = (reqUser?.username || 'U').slice(0, 2).toUpperCase();
              ...
              {loadingFriends ? (
                ...
                <div className="space-y-4">
                      ...
                      {loadingSuggestions ? (
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto scrollbar-hide">
                          {suggestedFriends.map((suggested) => {
                            /*SecurityHandlers*/ const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; /*SecurityHandlers*/ const initials = (suggested.username || 'U').slice(0, 2).toUpperCase();
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto scrollbar-hide">
                    {friends.map((friend) => {
                      /*SecurityHandlers*/ const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; /*SecurityHandlers*/ const initials = (friend.username || 'U').slice(0, 2).toUpperCase();
      ...
      </AlertDialog> <AlertDialog open={showPinModal} onOpenChange={setShowPinModal}> <AlertDialogContent className="max-w-xs bg-white dark:bg-[#1E293B] discuss:bg-[#1a1a1a] border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333]"> <AlertDialogHeader> <AlertDialogTitle className="text-center">Set Security PIN</AlertDialogTitle> <AlertDialogDescription className="text-center text-[11px]"> Choose a 6-digit PIN. This is only for this device. </AlertDialogDescription> </AlertDialogHeader> <div className="space-y-4 py-4"> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">New PIN</label> <Input type="password" maxLength={6} value={newPin} onChange={e => setNewPin(e.target.value.replace(/\D/g, ''))} placeholder="������" className="text-center text-xl tracking-[1em] font-mono" /> </div> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">Confirm PIN</label> <Input type="password" maxLength={6} value={confirmPin} onChange={e => setConfirmPin(e.target.value.replace(/\D/g, ''))} placeholder="������" className="text-center text-xl tracking-[1em] font-mono" /> </div> </div> <AlertDialogFooter className="flex flex-col gap-2"> <Button onClick={handleSavePinAndEnable} className="w-full bg-[#2563EB] discuss:bg-[#EF4444] text-white"> Save & Enable </Button> <Button variant="ghost" onClick={() => setShowPinModal(false)} className="w-full">Cancel</Button> </AlertDialogFooter> </AlertDialogContent> </AlertDialog> <AlertDialog open={showChangePinModal} onOpenChange={setShowChangePinModal}> <AlertDialogContent className="max-w-xs bg-white dark:bg-[#1E293B] discuss:bg-[#1a1a1a] border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333]"> <AlertDialogHeader> <AlertDialogTitle className="text-center">Change Security PIN</AlertDialogTitle> </AlertDialogHeader> <div className="space-y-4 py-4"> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">Old PIN</label> <Input type="password" maxLength={6} value={oldPin} onChange={e => setOldPin(e.target.value.replace(/\D/g, ''))} placeholder="������" className="text-center text-xl tracking-[1em] font-mono" /> </div> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">New PIN</label> <Input type="password" maxLength={6} value={newPin} onChange={e => setNewPin(e.target.value.replace(/\D/g, ''))} placeholder="������" className="text-center text-xl tracking-[1em] font-mono" /> </div> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">Confirm New PIN</label> <Input type="password" maxLength={6} value={confirmPin} onChange={e => setConfirmPin(e.target.value.replace(/\D/g, ''))} placeholder="������" className="text-center text-xl tracking-[1em] font-mono" /> </div> </div> <AlertDialogFooter className="flex flex-col gap-2"> <Button onClick={handleUpdatePin} className="w-full bg-[#2563EB] discuss:bg-[#EF4444] text-white"> Update PIN </Button> <Button variant="ghost" onClick={() => setShowChangePinModal(false)} className="w-full">Cancel</Button> </AlertDialogFooter> </AlertDialogContent> </AlertDialog>
