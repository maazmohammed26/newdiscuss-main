...
export default function ProfilePage() {
  ...
  const handleSaveTelegram = async () => {
    ...
    if (!trimmed) return;
    if (!/^\-?\d+$/.test(trimmed)) {
      toast.error('Invalid Chat ID ‚Äî must be a number (e.g. 123456789)');
      return;
    }
  ...
  const handleToggleSecurity = async () => { if (!settings.enabled) { setShowPinModal(true); } else { updateSettings({ enabled: false, type: 'none', pin: null }); toast.success('App lock disabled'); } }; const handleSavePinAndEnable = () => { if (newPin.length !== 6) { toast.error('PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ enabled: true, type: 'pin', pin: newPin }); setShowPinModal(false); setNewPin(''); setConfirmPin(''); toast.success('App lock enabled with PIN'); }; const handleEnableBiometric = async () => { setTestingBiometric(true); const success = await registerBiometric(user?.username || 'User'); if (success) { updateSettings({ type: 'biometric' }); toast.success('Biometric lock enabled'); } else { toast.error('Failed to register biometrics'); } setTestingBiometric(false); }; /*SecurityHandlers*/ const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; /*SecurityHandlers*/ const handleToggleSecurity = async () => { if (!settings.enabled) { setShowPinModal(true); } else { updateSettings({ enabled: false, type: 'none', pin: null }); toast.success('App lock disabled'); } }; const handleSavePinAndEnable = () => { if (newPin.length !== 6) { toast.error('PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ enabled: true, type: 'pin', pin: newPin }); setShowPinModal(false); setNewPin(''); setConfirmPin(''); toast.success('App lock enabled with PIN'); }; const handleEnableBiometric = async () => { setTestingBiometric(true); const success = await registerBiometric(user?.username || 'User'); if (success) { updateSettings({ type: 'biometric' }); toast.success('Biometric lock enabled'); } else { toast.error('Failed to register biometrics'); } setTestingBiometric(false); }; const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; const initials = (user?.username || 'U').slice(0, 2).toUpperCase();
    ...
    <div className="min-h-screen bg-[#F5F5F7] dark:bg-[#0F172A] discuss:bg-[#121212]">
      ...
      <div className="max-w-xl mx-auto px-4 py-6 md:py-10">
        ...
        <div className="bg-white dark:bg-[#1E293B] discuss:bg-[#1a1a1a] shadow-[0_4px_24px_rgba(0,0,0,0.06)] dark:shadow-none discuss:shadow-none border discuss:border discuss:border-[#333333] p-8 text-center relative rounded-2xl">
          ...
          {/* ==================== END PROFILE FIELDS ==================== */} <div className="mt-6 pt-5 border-t border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333]"> <div className="flex items-center gap-2 mb-4"> <Shield className="w-4 h-4 text-[#2563EB] discuss:text-[#EF4444]" /> <span className="text-[#0F172A] dark:text-[#F1F5F9] discuss:text-[#F5F5F5] text-sm font-medium">App Security</span> </div> <div className="space-y-4"> <div className="flex items-center justify-between bg-[#F5F5F7] dark:bg-[#0F172A] discuss:bg-[#1a1a1a] discuss:border discuss:border-[#333333] rounded-xl p-4"> <div className="flex items-center gap-3"> <div className={`p-2 rounded-lg ${settings.enabled ? 'bg-[#2563EB]/10 text-[#2563EB] discuss:bg-[#EF4444]/10 discuss:text-[#EF4444]' : 'bg-[#6275AF]/10 text-[#6275AF]'}`}> <Smartphone className="w-5 h-5" /> </div> <div> <p className="text-sm font-semibold text-[#0F172A] dark:text-[#F1F5F9] discuss:text-[#F5F5F5]">App Lock</p> <p className="text-[11px] text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF]">PIN or Biometrics on launch</p> </div> </div> <button onClick={handleToggleSecurity} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${settings.enabled ? 'bg-[#2563EB] discuss:bg-[#EF4444]' : 'bg-neutral-200 dark:bg-neutral-700'}`} > <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.enabled ? 'translate-x-6' : 'translate-x-1'}`} /> </button> </div> {settings.enabled && ( <div className="space-y-2 animate-in slide-in-from-top-2 duration-300"> {biometricAvailable && ( <div className="flex items-center justify-between border border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333] rounded-xl p-4"> <div className="flex items-center gap-3"> <div className={`p-2 rounded-lg ${settings.type === 'biometric' ? 'bg-[#2563EB]/10 text-[#2563EB] discuss:bg-[#EF4444]/10 discuss:text-[#EF4444]' : 'bg-[#6275AF]/10 text-[#6275AF]'}`}> <BiometricIcon className="w-5 h-5" /> </div> <div> <p className="text-sm font-semibold text-[#0F172A] dark:text-[#F1F5F9] discuss:text-[#F5F5F5]">Biometrics</p> <p className="text-[11px] text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF]">Use FaceID/Fingerprint</p> </div> </div> <button onClick={handleEnableBiometric} disabled={testingBiometric} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${settings.type === 'biometric' ? 'bg-[#2563EB] discuss:bg-[#EF4444]' : 'bg-neutral-200 dark:bg-neutral-700'}`} > <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.type === 'biometric' ? 'translate-x-6' : 'translate-x-1'}`} /> </button> </div> )} <Button onClick={() => setShowChangePinModal(true)} variant="outline" size="sm" className="w-full text-xs text-[#6275AF]"> Change Security PIN </Button> </div> )} <div className="flex items-center gap-2 p-3 bg-blue-500/5 rounded-lg border border-blue-500/10"> <Info className="w-3.5 h-3.5 text-blue-500" /> <p className="text-[10px] text-blue-600 dark:text-blue-400"> Note: This lock is only for this device. Other devices won't be affected. </p> </div> </div> </div>
          ...
          <div className="mt-6 pt-5 border-t border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333]">
            ...
            {showTelegramInstructions && (
              <div className="mb-3 bg-[#229ED9]/5 border border-[#229ED9]/20 rounded-lg p-4 text-xs text-[#0F172A] dark:text-[#F1F5F9] discuss:text-[#F5F5F5] space-y-3">
                ...
                <div className="space-y-2">
                  <p className="font-medium text-[#0F172A] dark:text-[#F1F5F9] discuss:text-[#F5F5F5]">Step 1 ‚Äî Open the bot</p>
                ...
                <div className="space-y-1">
                  <p className="font-medium text-[#0F172A] dark:text-[#F1F5F9] discuss:text-[#F5F5F5]">Step 2 ‚Äî Start the bot</p>
                  <p className="text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF]">
                    Tap <span className="font-semibold">Start</span> or send <span className="font-mono font-semibold">/start</span>. The bot will instantly reply with your <span className="font-semibold">Id</span> ‚Äî a number like <span className="font-mono">123456789</span>.
                  </p>
                </div>

                <div className="space-y-1">
                  <p className="font-medium text-[#0F172A] dark:text-[#F1F5F9] discuss:text-[#F5F5F5]">Step 3 ‚Äî Paste your Chat ID below</p>
                  <p className="text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF]">
                ...
                <div className="pt-2 border-t border-[#229ED9]/20">
                  ...
                  <ul className="text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF] space-y-0.5">
                    <li>üí¨ New direct messages</li>
                    <li>üë• Friend requests &amp; acceptances</li>
                    <li>üí¨ Group chat messages</li>
                    <li>‚úÖ Group join approvals</li>
                  </ul>
                </div>
            ...
            {loadingTelegram ? (
              ...
              <div className="space-y-2">
                <p className="text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF] text-xs">
                  Paste your Telegram Chat ID to receive notifications (tap ‚ÑπÔ∏è for instructions).
                </p>
                <div className="flex gap-2">
                  <Input
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF] text-xs">
                  ...
                  {receivedRequests.length > 0 && ` ‚Ä¢ ${receivedRequests.length} pending`}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-4">
                ...
                <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/20 rounded-xl p-4">
                  ...
                  <div className="space-y-2">
                    {receivedRequests.map((request) => {
                      ...
                      /*SecurityHandlers*/ const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; /*SecurityHandlers*/ const handleToggleSecurity = async () => { if (!settings.enabled) { setShowPinModal(true); } else { updateSettings({ enabled: false, type: 'none', pin: null }); toast.success('App lock disabled'); } }; const handleSavePinAndEnable = () => { if (newPin.length !== 6) { toast.error('PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ enabled: true, type: 'pin', pin: newPin }); setShowPinModal(false); setNewPin(''); setConfirmPin(''); toast.success('App lock enabled with PIN'); }; const handleEnableBiometric = async () => { setTestingBiometric(true); const success = await registerBiometric(user?.username || 'User'); if (success) { updateSettings({ type: 'biometric' }); toast.success('Biometric lock enabled'); } else { toast.error('Failed to register biometrics'); } setTestingBiometric(false); }; const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; const initials = (reqUser?.username || 'U').slice(0, 2).toUpperCase();
              ...
              {loadingFriends ? (
                ...
                <div className="space-y-4">
                      ...
                      {loadingSuggestions ? (
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto scrollbar-hide">
                          {suggestedFriends.map((suggested) => {
                            /*SecurityHandlers*/ const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; /*SecurityHandlers*/ const handleToggleSecurity = async () => { if (!settings.enabled) { setShowPinModal(true); } else { updateSettings({ enabled: false, type: 'none', pin: null }); toast.success('App lock disabled'); } }; const handleSavePinAndEnable = () => { if (newPin.length !== 6) { toast.error('PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ enabled: true, type: 'pin', pin: newPin }); setShowPinModal(false); setNewPin(''); setConfirmPin(''); toast.success('App lock enabled with PIN'); }; const handleEnableBiometric = async () => { setTestingBiometric(true); const success = await registerBiometric(user?.username || 'User'); if (success) { updateSettings({ type: 'biometric' }); toast.success('Biometric lock enabled'); } else { toast.error('Failed to register biometrics'); } setTestingBiometric(false); }; const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; const initials = (suggested.username || 'U').slice(0, 2).toUpperCase();
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto scrollbar-hide">
                    {friends.map((friend) => {
                      /*SecurityHandlers*/ const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; /*SecurityHandlers*/ const handleToggleSecurity = async () => { if (!settings.enabled) { setShowPinModal(true); } else { updateSettings({ enabled: false, type: 'none', pin: null }); toast.success('App lock disabled'); } }; const handleSavePinAndEnable = () => { if (newPin.length !== 6) { toast.error('PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ enabled: true, type: 'pin', pin: newPin }); setShowPinModal(false); setNewPin(''); setConfirmPin(''); toast.success('App lock enabled with PIN'); }; const handleEnableBiometric = async () => { setTestingBiometric(true); const success = await registerBiometric(user?.username || 'User'); if (success) { updateSettings({ type: 'biometric' }); toast.success('Biometric lock enabled'); } else { toast.error('Failed to register biometrics'); } setTestingBiometric(false); }; const handleUpdatePin = () => { if (oldPin !== settings.pin) { toast.error('Incorrect old PIN'); return; } if (newPin.length !== 6) { toast.error('New PIN must be 6 digits'); return; } if (newPin !== confirmPin) { toast.error('PINs do not match'); return; } updateSettings({ pin: newPin }); setShowChangePinModal(false); setOldPin(''); setNewPin(''); setConfirmPin(''); toast.success('PIN updated successfully'); }; const initials = (friend.username || 'U').slice(0, 2).toUpperCase();
      ...
      </AlertDialog> <AlertDialog open={showPinModal} onOpenChange={setShowPinModal}> <AlertDialogContent className="max-w-xs bg-white dark:bg-[#1E293B] discuss:bg-[#1a1a1a] border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333]"> <AlertDialogHeader> <AlertDialogTitle className="text-center">Set Security PIN</AlertDialogTitle> <AlertDialogDescription className="text-center text-[11px]"> Choose a 6-digit PIN. This is only for this device. </AlertDialogDescription> </AlertDialogHeader> <div className="space-y-4 py-4"> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">New PIN</label> <Input type="password" maxLength={6} value={newPin} onChange={e => setNewPin(e.target.value.replace(/\D/g, ''))} placeholder="ïïïïïï" className="text-center text-xl tracking-[1em] font-mono" /> </div> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">Confirm PIN</label> <Input type="password" maxLength={6} value={confirmPin} onChange={e => setConfirmPin(e.target.value.replace(/\D/g, ''))} placeholder="ïïïïïï" className="text-center text-xl tracking-[1em] font-mono" /> </div> </div> <AlertDialogFooter className="flex flex-col gap-2"> <Button onClick={handleSavePinAndEnable} className="w-full bg-[#2563EB] discuss:bg-[#EF4444] text-white"> Save & Enable </Button> <Button variant="ghost" onClick={() => setShowPinModal(false)} className="w-full">Cancel</Button> </AlertDialogFooter> </AlertDialogContent> </AlertDialog> <AlertDialog open={showChangePinModal} onOpenChange={setShowChangePinModal}> <AlertDialogContent className="max-w-xs bg-white dark:bg-[#1E293B] discuss:bg-[#1a1a1a] border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333]"> <AlertDialogHeader> <AlertDialogTitle className="text-center">Change Security PIN</AlertDialogTitle> </AlertDialogHeader> <div className="space-y-4 py-4"> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">Old PIN</label> <Input type="password" maxLength={6} value={oldPin} onChange={e => setOldPin(e.target.value.replace(/\D/g, ''))} placeholder="ïïïïïï" className="text-center text-xl tracking-[1em] font-mono" /> </div> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">New PIN</label> <Input type="password" maxLength={6} value={newPin} onChange={e => setNewPin(e.target.value.replace(/\D/g, ''))} placeholder="ïïïïïï" className="text-center text-xl tracking-[1em] font-mono" /> </div> <div className="space-y-2"> <label className="text-[11px] font-medium text-[#6275AF]">Confirm New PIN</label> <Input type="password" maxLength={6} value={confirmPin} onChange={e => setConfirmPin(e.target.value.replace(/\D/g, ''))} placeholder="ïïïïïï" className="text-center text-xl tracking-[1em] font-mono" /> </div> </div> <AlertDialogFooter className="flex flex-col gap-2"> <Button onClick={handleUpdatePin} className="w-full bg-[#2563EB] discuss:bg-[#EF4444] text-white"> Update PIN </Button> <Button variant="ghost" onClick={() => setShowChangePinModal(false)} className="w-full">Cancel</Button> </AlertDialogFooter> </AlertDialogContent> </AlertDialog>
