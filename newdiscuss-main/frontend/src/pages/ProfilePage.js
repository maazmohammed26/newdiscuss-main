...
import { analyzeUserProfile, discoverUserSkills } from '@/lib/ai';
import {
  getReceivedRequests,
  getSentRequests,
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {!editingSkills && skills.length > 0 && (
                  <button onClick={() => { setEditingSkills(true); setSelectedSkillsInput(skills); }}
                    className="p-1.5 rounded hover:bg-[#E2E8F0] dark:hover:bg-[#1E293B] discuss:hover:bg-[#1a1a1a] text-[#6275AF] dark:text-[#94A3B8] discuss:text-[#9CA3AF] transition-colors">
                ...
                <div className="space-y-3">
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {PREDEFINED_SKILLS.map(s => {
                      ...
                      return (
                        ...
                        className={`px-2 py-1 rounded text-xs font-medium border ${isSel ? 'bg-neutral-900 text-white border-neutral-900 dark:bg-white dark:text-neutral-900' : 'bg-transparent text-neutral-600 border-neutral-200 dark:text-neutral-400 dark:border-neutral-800'}`}>
                          {s}
                        </button>
                      );
                    })}
                  ...
                  <div className="flex gap-2">
                    <Input 
                      ...
                      maxLength={20}
                    />
                    <Button onClick={handleAddCustomSkillInProfile} size="sm" variant="outline" className="h-8 text-xs border-[#E2E8F0]">Add</Button>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {selectedSkillsInput.map(s => (
                        <span key={s} className="inline-flex items-center gap-1 bg-neutral-100 dark:bg-neutral-800 text-neutral-800 dark:text-neutral-200 px-2 py-0.5 rounded text-xs">
                          {s}
                          <button type="button" onClick={() => setSelectedSkillsInput(prev => prev.filter(x => x !== s))} className="text-red-500 font-bold ml-1">&times;</button>
                        </span>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex gap-2 justify-end pt-2">
                    <Button onClick={handleSaveSkills} disabled={savingSkills} size="sm" className="bg-[#2563EB] discuss:bg-[#EF4444] text-white">
                      Save
                    </Button>
                    <Button onClick={() => setEditingSkills(false)} size="sm" variant="outline" className="border-[#E2E8F0]">
                      Cancel
                    </Button>
                  </div>
                </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {suggestedSkills.map(s => (
                          <button key={s} onClick={() => handleAddSuggestedSkill(s)}
                            className="bg-white dark:bg-neutral-850 hover:bg-neutral-100 text-[#2563EB] dark:text-[#60A5FA] px-2 py-0.5 rounded text-xs border border-blue-200 dark:border-blue-900 font-semibold transition-colors">
                            + {s}
                          </button>
                        ))}
                      </div>
                      <button onClick={() => setSuggestedSkills([])} className="text-[10px] text-neutral-400 hover:text-neutral-600 block mt-1">Dismiss suggestions</button>
                    </div>
                  ) : (
                    <button
                      onClick={handleDiscoverSkills}
                      disabled={discoveringSkills}
                      className="text-xs text-neutral-500 hover:text-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-200 flex items-center gap-1 transition-colors"
                    >
                      {discoveringSkills ? (
                        <>
                          <Loader2 className="w-3 h-3 animate-spin" />
                          Analyzing posts for skills...
                        </>
                      ) : (
                        "Analyze posts to discover skills with AI"
                      )}
                    </button>
                  )}
                </div>
            ...
            <div className="mt-4 bg-white dark:bg-[#1E293B] discuss:bg-[#1a1a1a] border border-[#E2E8F0] dark:border-[#334155] discuss:border-[#333333] rounded-xl p-5 space-y-4">
              <div className="flex justify-between items-center pb-3 border-b border-neutral-100 dark:border-neutral-800">
                ...
                <Button
                  ...
                  size="sm"
                  className="bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 font-semibold text-xs py-1.5 px-3 rounded-md border border-neutral-200 dark:border-neutral-800"
                >
                  {analyzingProfile ? (
                    <>
              ...
              {aiInsights ? (
                ...
                <div className="text-center py-6">
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-4">Analyze your profile bio and posts to extract key technical domains, roles, and growth areas.</p>
                  <Button
                    onClick={handleAnalyzeProfile}
                    disabled={analyzingProfile}
                    className="bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 font-semibold text-xs py-2 px-4 rounded-md border border-neutral-200"
                  >
                    {analyzingProfile ? (
                      <>
